...
void CFilmGrainEffect::Render( int x, int y, int w, int h )
{
	...
	if( m_bEnableFlicker )
	{
		IMaterial *pFlickerMaterial = materials->FindMaterial( "effects/flicker_128", TEXTURE_GROUP_OTHER, false );
		if( !IsErrorMaterial( pFlickerMaterial ) )
		{
			pFlickerMaterial->Refresh();
			pFlickerMaterial->SetMaterialVarFlag( MATERIAL_VAR_VERTEXALPHA, true );
			pRenderContext->Bind( pFlickerMaterial );
