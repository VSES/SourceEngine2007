...
//	TheBots->OnEvent( EVENT_PLAYER_DIED, this, info.GetAttacker() );
	...
	if ( GetTeamNumber() != pScorer->GetTeamNumber() && pInflictor != pScorer )
	{
		...
		if ( strncmp( killer_weapon_name, "rocket_", 7 ) == 0 )
		{
			...
			if ( m_Shared.IsInMGDeploy() || m_Shared.IsSniperZoomed() )
			{
				if ( flDist > ACHIEVEMENT_LONG_RANGE_ROCKET_DIST )
				{
...
void CDODPlayer::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
	...
	if ( pAttacker && ( GetTeamNumber() == pAttacker->GetTeamNumber() ) )
	 	 bTakeDamage = DODGameRules()->IsFriendlyFireOn();
	...
	if( bTakeDamage && pAttacker->GetTeamNumber() != GetTeamNumber() || DODGameRules()->IsFriendlyFireOn() )
	{
		m_Shared.SetSlowedTime( 0.5f );
	}

	/*
	//rockets are bad for people
	if( info.GetDamageType() & DMG_ROCKET )
	{	
		flDamage *= 1.5;
	}
	*/
	...
	if( bTakeDamage && dod_debugdamage.GetInt() )
	{
		char buf[256];
...
int CDODPlayer::GetNearestLocationAsString( char *pDest, int iDestSize )
{
	...
	Q_snprintf( pDest, iDestSize, pLocationName );
	return Q_strlen(pDest);
}
...
void CDODPlayer::State_Enter_WELCOME()
{
	...
	else
	{
		...
		data->SetString( "cmd", COMMAND_CHANGE_TEAM );  // exec this command if panel closed
...
bool CDODPlayer::SelectSpawnSpot( const char *pEntClassName, CBaseEntity* &pSpot )
{
	...
	do 
	{
		if ( pSpot )
		{
			Assert( strcmp( STRING(pSpot->m_iClassname), pEntClassName ) == 0 );
	...
	do
	{
		...
		for( int i=0;i<4;i++ )
		{
			...
			if ( UTIL_IsSpaceEmpty( this, vTestMins, vTestMaxs ) )
			{
                ...
                pSpot = CreateEntityByName( pEntClassName );
	...
	case TEAM_ALLIES:
		{
			...
			pSpot = g_pLastAlliesSpawn;
			if ( SelectSpawnSpot( pSpawnPointName, pSpot ) )
			{
				g_pLastAlliesSpawn = pSpot;
			}
		}
		break;
	case TEAM_AXIS:
		{
			...
			pSpot = g_pLastAxisSpawn;
			if ( SelectSpawnSpot( pSpawnPointName, pSpot ) )
			{
				g_pLastAxisSpawn = pSpot;
			}
		}		
		break;
	case TEAM_SPECTATOR:
...
void CDODPlayer::OutputDamageTaken( void )
{
	...
	FOR_EACH_LL( m_DamageTakenList, i )
	{
		...
		if( pRecord )
		{
			Q_snprintf( buf, sizeof(buf), "( life ID %i ) - %d in %d hit",
				pRecord->GetLifeID(),
				pRecord->GetDamage(),
				pRecord->GetNumHits(),
...
void CDODPlayer::CommitSuicide( void )
{
	if( !IsAlive() )
		return;
...
void CDODPlayer::SetClassMenuOpen( bool bOpen )
{
	m_bIsClassMenuOpen = bOpen;
}

bool CDODPlayer::IsClassMenuOpen( void )
{
...
void CDODPlayer::HandleHeadshotAchievement( int iNumHeadshots )
{
	...
	{
		m_iConsecutiveHeadshots += iNumHeadshots;

		if ( m_iConsecutiveHeadshots >= ACHIEVEMENT_NUM_CONSECUTIVE_HEADSHOTS )
		{
...
void CDODPlayer::HandleDeployedMGKillCount( int iNumDeployedKills )
{
	...
	{
		m_iKillsFromMGPosition = 0;
	}
	else
	{
		m_iKillsFromMGPosition += iNumDeployedKills;

		if ( m_iKillsFromMGPosition >= ACHIEVEMENT_NUM_DEPLOYED_KILLS )
		{
...
void CDODPlayer::HandleEnemyWeaponsAchievement( int iNumEnemyWpnKills )
{
	...
	{
		m_iEnemyWeaponKills = 0;
	}
	else
	{
		m_iEnemyWeaponKills += iNumEnemyWpnKills;

		if ( m_iEnemyWeaponKills >= ACHIEVEMENT_NUM_ENEMY_WPN_KILLS )
		{
...
{
	m_iComboWeaponKillMask = 0;
}

void CDODPlayer::HandleComboWeaponKill( int iWeaponType )
{
	m_iComboWeaponKillMask |= iWeaponType;
	...
	if ( ( m_iComboWeaponKillMask & iRequiredMask ) == iRequiredMask ) // must have all these bits set
	{
		if ( ( m_iComboWeaponKillMask & iExplosiveMask ) > 0 )	// has one of these bits set
		{
