...
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Water_ps14.vsh)(5)\n";
push @output, "; Constants specified by the app																								; LINEINFO(Water_ps14.vsh)(6)\n";
     push @output, ";    c0      = (0, 1, 2, 0.5)																								; LINEINFO(Water_ps14.vsh)(7)\n";
 	 push @output, ";	 c1		 = (1/2.2, 0, 0, 0)																								; LINEINFO(Water_ps14.vsh)(8)\n";
     push @output, ";    c2      = camera position *in world space*																								; LINEINFO(Water_ps14.vsh)(9)\n";
     push @output, ";    c4-c7   = modelViewProj matrix	(transpose)																								; LINEINFO(Water_ps14.vsh)(10)\n";
     push @output, ";    c8-c11  = ViewProj matrix (transpose)																								; LINEINFO(Water_ps14.vsh)(11)\n";
     push @output, ";    c12-c15 = model->view matrix (transpose)																								; LINEINFO(Water_ps14.vsh)(12)\n";
 	 push @output, ";	 c16	 = [fogStart, fogEnd, fogRange, undefined]																								; LINEINFO(Water_ps14.vsh)(13)\n";
 																								push @output, ";																								; LINEINFO(Water_ps14.vsh)(14)\n";
push @output, "; $SHADER_SPECIFIC_CONST_0..$SHADER_SPECIFIC_CONST_3 - special proj matrix																								; LINEINFO(Water_ps14.vsh)(15)\n";
 																								push @output, ";																								; LINEINFO(Water_ps14.vsh)(16)\n";
push @output, "; Vertex components (as specified in the vertex DECL)																								; LINEINFO(Water_ps14.vsh)(17)\n";
     push @output, ";    $vPos    = Position																								; LINEINFO(Water_ps14.vsh)(18)\n";
 	 push @output, ";	 $vTexCoord0.xy = TexCoord0																								; LINEINFO(Water_ps14.vsh)(19)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Water_ps14.vsh)(20)\n";
...
push @output, "; Vertex components																								; LINEINFO(Water_ps14.vsh)(24)\n";
     push @output, ";    $vPos		= Position																								; LINEINFO(Water_ps14.vsh)(25)\n";
 	 push @output, ";	 $vNormal		= normal																								; LINEINFO(Water_ps14.vsh)(26)\n";
 	 push @output, ";	 $vTexCoord0.xy	= TexCoord0																								; LINEINFO(Water_ps14.vsh)(27)\n";
 	 push @output, ";	 $vTangentS		= S axis of Texture space																								; LINEINFO(Water_ps14.vsh)(28)\n";
 	 push @output, ";	 $vTangentT	= T axis of Texture space																								; LINEINFO(Water_ps14.vsh)(29)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Water_ps14.vsh)(32)\n";
push @output, "; Transform the position from world to view space																								; LINEINFO(Water_ps14.vsh)(33)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Water_ps14.vsh)(34)\n";
local( $projPos ); &AllocateRegister( \$projPos, "\$projPos" );
push @output, "; Transform position from object to projection space																								; LINEINFO(Water_ps14.vsh)(39)\n";
push @output, "dp4 $projPos.x, $vPos, $cModelViewProj0																								; LINEINFO(Water_ps14.vsh)(40)\n";
push @output, "dp4 $projPos.y, $vPos, $cModelViewProj1																								; LINEINFO(Water_ps14.vsh)(41)\n";
push @output, "dp4 $projPos.z, $vPos, $cModelViewProj2																								; LINEINFO(Water_ps14.vsh)(42)\n";
push @output, "dp4 $projPos.w, $vPos, $cModelViewProj3																								; LINEINFO(Water_ps14.vsh)(43)\n";
push @output, "mov oPos, $projPos																								; LINEINFO(Water_ps14.vsh)(44)\n";
local( $worldPos ); &AllocateRegister( \$worldPos, "\$worldPos" );
push @output, "; Transform position from object to world space																								; LINEINFO(Water_ps14.vsh)(48)\n";
push @output, "dp4 $worldPos.x, $vPos, $cModel0																								; LINEINFO(Water_ps14.vsh)(49)\n";
push @output, "dp4 $worldPos.y, $vPos, $cModel1																								; LINEINFO(Water_ps14.vsh)(50)\n";
push @output, "dp4 $worldPos.z, $vPos, $cModel2																								; LINEINFO(Water_ps14.vsh)(51)\n";
&CalcFog( $worldPos, $projPos );																								
local( $worldEyeVect ); &AllocateRegister( \$worldEyeVect, "\$worldEyeVect" );
push @output, "; Get the eye vector in world space																								; LINEINFO(Water_ps14.vsh)(57)\n";
push @output, "add $worldEyeVect.xyz, -$worldPos, $cEyePos																								; LINEINFO(Water_ps14.vsh)(58)\n";
local( $tangentEyeVect ); &AllocateRegister( \$tangentEyeVect, "\$tangentEyeVect" );
push @output, "; transform the eye vector to tangent space																								; LINEINFO(Water_ps14.vsh)(62)\n";
push @output, "dp3 $tangentEyeVect.x, $worldEyeVect, $vTangentS																								; LINEINFO(Water_ps14.vsh)(63)\n";
push @output, "dp3 $tangentEyeVect.y, $worldEyeVect, $vTangentT																								; LINEINFO(Water_ps14.vsh)(64)\n";
push @output, "dp3 $tangentEyeVect.z, $worldEyeVect, $vNormal																								; LINEINFO(Water_ps14.vsh)(65)\n";
push @output, "mov $tangentEyeVect.w, $cZero																								; LINEINFO(Water_ps14.vsh)(66)\n";
push @output, "mov oT5, $tangentEyeVect																								; LINEINFO(Water_ps14.vsh)(68)\n";
push @output, "; base coordinates																								; LINEINFO(Water_ps14.vsh)(70)\n";
push @output, "dp4 oT0.x, $vTexCoord0, $SHADER_SPECIFIC_CONST_1																								; LINEINFO(Water_ps14.vsh)(71)\n";
push @output, "dp4 oT0.y, $vTexCoord0, $SHADER_SPECIFIC_CONST_2																								; LINEINFO(Water_ps14.vsh)(72)\n";
push @output, "; reflection																								; LINEINFO(Water_ps14.vsh)(74)\n";
local( $projPosReflect ); &AllocateRegister( \$projPosReflect, "\$projPosReflect" );
push @output, "mov $projPosReflect, $projPos																								; LINEINFO(Water_ps14.vsh)(77)\n";
push @output, "add $projPosReflect.xy, $projPosReflect, $projPosReflect.w																								; LINEINFO(Water_ps14.vsh)(78)\n";
push @output, "mul $projPosReflect.xy, $projPosReflect, $cHalf																								; LINEINFO(Water_ps14.vsh)(79)\n";
push @output, "mov oT1, $projPosReflect																								; LINEINFO(Water_ps14.vsh)(80)\n";
push @output, "; refraction																								; LINEINFO(Water_ps14.vsh)(82)\n";
push @output, "mov $projPos.y, -$projPos.y																								; LINEINFO(Water_ps14.vsh)(83)\n";
push @output, "add $projPos.xy, $projPos, $projPos.w																								; LINEINFO(Water_ps14.vsh)(84)\n";
push @output, "mul $projPos.xy, $projPos, $cHalf																								; LINEINFO(Water_ps14.vsh)(85)\n";
push @output, "mov oT2, $projPos																								; LINEINFO(Water_ps14.vsh)(86)\n";
push @output, "; reflectionscale, refractionscale																								; LINEINFO(Water_ps14.vsh)(88)\n";
push @output, "mov oT4, $SHADER_SPECIFIC_CONST_4																								; LINEINFO(Water_ps14.vsh)(89)\n";
