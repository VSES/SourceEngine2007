...
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Cable.vsh)(7)\n";
push @output, "; The cable equation is:																								; LINEINFO(Cable.vsh)(8)\n";
push @output, "; [L dot N] * C * T																								; LINEINFO(Cable.vsh)(9)\n";
 																								push @output, ";																								; LINEINFO(Cable.vsh)(10)\n";
push @output, "; where:																								; LINEINFO(Cable.vsh)(11)\n";
push @output, "; C = directional light color																								; LINEINFO(Cable.vsh)(12)\n";
push @output, "; T = baseTexture																								; LINEINFO(Cable.vsh)(13)\n";
push @output, "; N = particle normal (stored in the normal map)																								; LINEINFO(Cable.vsh)(14)\n";
push @output, "; L = directional light direction																								; LINEINFO(Cable.vsh)(15)\n";
 																								push @output, ";																								; LINEINFO(Cable.vsh)(16)\n";
 	push @output, ";	$SHADER_SPECIFIC_CONST_0		= Directional light direction																								; LINEINFO(Cable.vsh)(17)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Cable.vsh)(18)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Cable.vsh)(21)\n";
push @output, "; Transform position from object to projection space																								; LINEINFO(Cable.vsh)(22)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Cable.vsh)(23)\n";
&AllocateRegister( \$projPos, "\$projPos" );																								
push @output, "dp4		$projPos.x, $vPos, $cModelViewProj0																								; LINEINFO(Cable.vsh)(27)\n";
push @output, "dp4		$projPos.y, $vPos, $cModelViewProj1																								; LINEINFO(Cable.vsh)(28)\n";
push @output, "dp4		$projPos.z, $vPos, $cModelViewProj2																								; LINEINFO(Cable.vsh)(29)\n";
push @output, "dp4		$projPos.w, $vPos, $cModelViewProj3																								; LINEINFO(Cable.vsh)(30)\n";
push @output, "mov		oPos, $projPos																								; LINEINFO(Cable.vsh)(32)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Cable.vsh)(35)\n";
push @output, "; Fog																								; LINEINFO(Cable.vsh)(36)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Cable.vsh)(37)\n";
local( $worldPos ); &AllocateRegister( \$worldPos, "\$worldPos" );
if( $DOWATERFOG == 1 )																								
{																								
	push @output, "	; Get the worldpos z component only since that's all we need for height fog																								; LINEINFO(Cable.vsh)(42)\n";
	push @output, "	dp4 $worldPos.z, $vPos, $cModel2																								; LINEINFO(Cable.vsh)(43)\n";
}																								
&CalcFog( $worldPos, $projPos );																								
&FreeRegister( \$worldPos, "\$worldPos" );
&FreeRegister( \$projPos, "\$projPos" );																								
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Cable.vsh)(50)\n";
push @output, "; Setup the tangent space																								; LINEINFO(Cable.vsh)(51)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Cable.vsh)(52)\n";
&AllocateRegister( \$tmp1, "\$tmp1" );																								
&AllocateRegister( \$tmp2, "\$tmp2" );																								
&AllocateRegister( \$tmp3, "\$tmp3" );																								
&AllocateRegister( \$r, "\$r" );																								
push @output, "; Get S crossed with T (call it R)																								; LINEINFO(Cable.vsh)(59)\n";
push @output, "mov		$tmp1, $vTangentS																								; LINEINFO(Cable.vsh)(60)\n";
push @output, "mov		$tmp2, $vTangentT																								; LINEINFO(Cable.vsh)(61)\n";
push @output, "mul		$tmp3, $vTangentS.yzxw,  $tmp2.zxyw																								; LINEINFO(Cable.vsh)(63)\n";
push @output, "mad		$r, -$vTangentS.zxyw, $tmp2.yzxw, $tmp3																								; LINEINFO(Cable.vsh)(64)\n";
&FreeRegister( \$tmp2, "\$tmp2" );																								
&FreeRegister( \$tmp3, "\$tmp3" );																								
&AllocateRegister( \$s, "\$s" );																								
push @output, "; Normalize S (into $s)																								; LINEINFO(Cable.vsh)(71)\n";
push @output, "dp3		$s.w, $vTangentS, $vTangentS																								; LINEINFO(Cable.vsh)(72)\n";
push @output, "rsq		$s.w, $s.w																								; LINEINFO(Cable.vsh)(73)\n";
push @output, "mul		$s.xyz, $vTangentS, $s.w																								; LINEINFO(Cable.vsh)(74)\n";
push @output, "; Normalize R (into $r)																								; LINEINFO(Cable.vsh)(76)\n";
push @output, "dp3		$r.w, $r, $r																								; LINEINFO(Cable.vsh)(77)\n";
push @output, "rsq		$r.w, $r.w																								; LINEINFO(Cable.vsh)(78)\n";
push @output, "mul		$r.xyz, $r, $r.w																								; LINEINFO(Cable.vsh)(79)\n";
&AllocateRegister( \$t, "\$t" );																								
push @output, "; Regenerate T (into $t)																								; LINEINFO(Cable.vsh)(83)\n";
push @output, "mul		$t, $r.yzxw,  $tmp1.zxyw																								; LINEINFO(Cable.vsh)(84)\n";
push @output, "mad		$t, -$r.zxyw, $tmp1.yzxw, $t																								; LINEINFO(Cable.vsh)(85)\n";
&FreeRegister( \$tmp1, "\$tmp1" );																								
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Cable.vsh)(89)\n";
push @output, "; Transform the light direction (into oD1)																								; LINEINFO(Cable.vsh)(90)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Cable.vsh)(91)\n";
&AllocateRegister( \$lightDirection, "\$lightDirection" );																								
push @output, "dp3		$lightDirection.x, $s, $SHADER_SPECIFIC_CONST_0																								; LINEINFO(Cable.vsh)(95)\n";
push @output, "dp3		$lightDirection.y, $t, $SHADER_SPECIFIC_CONST_0																								; LINEINFO(Cable.vsh)(96)\n";
push @output, "dp3		$lightDirection.z, $r, $SHADER_SPECIFIC_CONST_0																								; LINEINFO(Cable.vsh)(97)\n";
&FreeRegister( \$r, "\$r" );																								
&FreeRegister( \$s, "\$s" );																								
&FreeRegister( \$t, "\$t" );																								
push @output, "; Scale into 0-1 range (we're assuming light direction was normalized prior to here)																								; LINEINFO(Cable.vsh)(103)\n";
push @output, "add		oT2, $lightDirection, $cHalf	; + 0.5 																								; LINEINFO(Cable.vsh)(104)\n";
&FreeRegister( \$lightDirection, "\$lightDirection" );																								
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Cable.vsh)(107)\n";
push @output, "; Copy texcoords for the normal map and base texture																								; LINEINFO(Cable.vsh)(108)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Cable.vsh)(109)\n";
push @output, "mov		oT0, $vTexCoord0																								; LINEINFO(Cable.vsh)(111)\n";
push @output, "mov		oT1, $vTexCoord1																								; LINEINFO(Cable.vsh)(112)\n";
push @output, "; Pass the dirlight color through																								; LINEINFO(Cable.vsh)(114)\n";
push @output, "mov		oD0.xyzw, $vColor																								; LINEINFO(Cable.vsh)(115)\n";
