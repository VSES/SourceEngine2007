...
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Refract_model_vs11.vsh)(6)\n";
push @output, "; Constants specified by the app																								; LINEINFO(Refract_model_vs11.vsh)(7)\n";
     push @output, ";    c0      = (0, 1, 2, 0.5)																								; LINEINFO(Refract_model_vs11.vsh)(8)\n";
 	 push @output, ";	 c1		 = (1/2.2, 0, 0, 0)																								; LINEINFO(Refract_model_vs11.vsh)(9)\n";
     push @output, ";    c2      = camera position *in world space*																								; LINEINFO(Refract_model_vs11.vsh)(10)\n";
     push @output, ";    c4-c7   = modelViewProj matrix	(transpose)																								; LINEINFO(Refract_model_vs11.vsh)(11)\n";
     push @output, ";    c8-c11  = ViewProj matrix (transpose)																								; LINEINFO(Refract_model_vs11.vsh)(12)\n";
     push @output, ";    c12-c15 = model->view matrix (transpose)																								; LINEINFO(Refract_model_vs11.vsh)(13)\n";
 	 push @output, ";	 c16	 = [fogStart, fogEnd, fogRange, undefined]																								; LINEINFO(Refract_model_vs11.vsh)(14)\n";
 																								push @output, ";																								; LINEINFO(Refract_model_vs11.vsh)(15)\n";
push @output, "; Vertex components (as specified in the vertex DECL)																								; LINEINFO(Refract_model_vs11.vsh)(16)\n";
     push @output, ";    $vPos    = Position																								; LINEINFO(Refract_model_vs11.vsh)(17)\n";
 	 push @output, ";	 $vTexCoord0.xy = TexCoord0																								; LINEINFO(Refract_model_vs11.vsh)(18)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Refract_model_vs11.vsh)(19)\n";
...
push @output, "; Vertex components																								; LINEINFO(Refract_model_vs11.vsh)(23)\n";
     push @output, ";    $vPos		= Position																								; LINEINFO(Refract_model_vs11.vsh)(24)\n";
 	 push @output, ";	 $vNormal		= normal																								; LINEINFO(Refract_model_vs11.vsh)(25)\n";
 	 push @output, ";	 $vTexCoord0.xy	= TexCoord0																								; LINEINFO(Refract_model_vs11.vsh)(26)\n";
 	 push @output, ";	 $vTangentS		= S axis of Texture space																								; LINEINFO(Refract_model_vs11.vsh)(27)\n";
 	 push @output, ";	 $vTangentT	= T axis of Texture space																								; LINEINFO(Refract_model_vs11.vsh)(28)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Refract_model_vs11.vsh)(30)\n";
push @output, "; Transform the position from world to view space																								; LINEINFO(Refract_model_vs11.vsh)(31)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Refract_model_vs11.vsh)(32)\n";
...
push @output, "; Transform position from world to projection space																								; LINEINFO(Refract_model_vs11.vsh)(44)\n";
push @output, "dp4 $projPos.x, $worldPos, $cViewProj0																								; LINEINFO(Refract_model_vs11.vsh)(45)\n";
push @output, "dp4 $projPos.y, $worldPos, $cViewProj1																								; LINEINFO(Refract_model_vs11.vsh)(46)\n";
push @output, "dp4 $projPos.z, $worldPos, $cViewProj2																								; LINEINFO(Refract_model_vs11.vsh)(47)\n";
push @output, "dp4 $projPos.w, $worldPos, $cViewProj3																								; LINEINFO(Refract_model_vs11.vsh)(48)\n";
&CalcFog( $worldPos, $projPos );																								
local( $worldEyeVect ); &AllocateRegister( \$worldEyeVect, "\$worldEyeVect" );
push @output, "; Get the eye vector in world space																								; LINEINFO(Refract_model_vs11.vsh)(54)\n";
push @output, "add $worldEyeVect.xyz, -$worldPos, $cEyePos																								; LINEINFO(Refract_model_vs11.vsh)(55)\n";
local( $tangentEyeVect ); &AllocateRegister( \$tangentEyeVect, "\$tangentEyeVect" );
push @output, "; transform the eye vector to tangent space																								; LINEINFO(Refract_model_vs11.vsh)(58)\n";
push @output, "dp3 oT3.x, $worldEyeVect, $worldTangentS																								; LINEINFO(Refract_model_vs11.vsh)(59)\n";
push @output, "dp3 oT3.y, $worldEyeVect, $worldTangentT																								; LINEINFO(Refract_model_vs11.vsh)(60)\n";
push @output, "dp3 oT3.z, $worldEyeVect, $worldNormal																								; LINEINFO(Refract_model_vs11.vsh)(61)\n";
local( $bumpTexCoord ); &AllocateRegister( \$bumpTexCoord, "\$bumpTexCoord" );
push @output, "dp4 $bumpTexCoord.x, $vTexCoord0, $SHADER_SPECIFIC_CONST_1																								; LINEINFO(Refract_model_vs11.vsh)(65)\n";
push @output, "dp4 $bumpTexCoord.y, $vTexCoord0, $SHADER_SPECIFIC_CONST_2																								; LINEINFO(Refract_model_vs11.vsh)(66)\n";
push @output, "; dudv map																								; LINEINFO(Refract_model_vs11.vsh)(68)\n";
push @output, "mov oT0.xy, $bumpTexCoord																								; LINEINFO(Refract_model_vs11.vsh)(69)\n";
push @output, "; refract tint + alpha channel																								; LINEINFO(Refract_model_vs11.vsh)(71)\n";
push @output, "mov oT2.xy, $bumpTexCoord																								; LINEINFO(Refract_model_vs11.vsh)(72)\n";
push @output, "mov oT3.xy, $bumpTexCoord																								; LINEINFO(Refract_model_vs11.vsh)(73)\n";
&FreeRegister( \$bumpTexCoord, "\$bumpTexCoord" );
push @output, "mov oPos, $projPos																								; LINEINFO(Refract_model_vs11.vsh)(77)\n";
push @output, "; special case perspective correct texture projection so that the texture fits exactly on the screen																								; LINEINFO(Refract_model_vs11.vsh)(79)\n";
push @output, "; flip Y by multiplying by -1																								; LINEINFO(Refract_model_vs11.vsh)(81)\n";
push @output, "mul $projPos.y, $projPos.y, $SHADER_SPECIFIC_CONST_4.w																								; LINEINFO(Refract_model_vs11.vsh)(82)\n";
push @output, "; transform from [-w,w] to [0,2*w]																								; LINEINFO(Refract_model_vs11.vsh)(84)\n";
push @output, "; The reason this is w is because we are in perspective space/homogenous clip space.																								; LINEINFO(Refract_model_vs11.vsh)(85)\n";
push @output, "add $projPos.xy, $projPos.xy, $projPos.w																								; LINEINFO(Refract_model_vs11.vsh)(86)\n";
push @output, "; transform from [0,2*w] to [0,w]																								; LINEINFO(Refract_model_vs11.vsh)(88)\n";
push @output, "; We'll end up dividing by w in the pixel shader to get to [0,1]																								; LINEINFO(Refract_model_vs11.vsh)(89)\n";
push @output, "mul $projPos.xy, $projPos.xy, $cHalf																								; LINEINFO(Refract_model_vs11.vsh)(90)\n";
push @output, "mov oT1.xy, $projPos.xy																								; LINEINFO(Refract_model_vs11.vsh)(92)\n";
push @output, "; emit w to both z and w in case the driver screws up and divides by z																								; LINEINFO(Refract_model_vs11.vsh)(94)\n";
push @output, "mov oT1.z, $projPos.w																								; LINEINFO(Refract_model_vs11.vsh)(95)\n";
push @output, "mov oT1.w, $projPos.w																								; LINEINFO(Refract_model_vs11.vsh)(96)\n";
&FreeRegister( \$projPos, "\$projPos" );
