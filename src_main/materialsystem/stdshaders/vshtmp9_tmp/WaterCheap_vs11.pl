...
push @output, "; Vertex blending																								; LINEINFO(WaterCheap_vs11.vsh)(8)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(WaterCheap_vs11.vsh)(9)\n";
&AllocateRegister( \$worldPos, "\$worldPos" );																								
push @output, "; Transform position from object to world																								; LINEINFO(WaterCheap_vs11.vsh)(13)\n";
push @output, "dp4 $worldPos.x, $vPos, $cModel0																								; LINEINFO(WaterCheap_vs11.vsh)(14)\n";
push @output, "dp4 $worldPos.y, $vPos, $cModel1																								; LINEINFO(WaterCheap_vs11.vsh)(15)\n";
push @output, "dp4 $worldPos.z, $vPos, $cModel2																								; LINEINFO(WaterCheap_vs11.vsh)(16)\n";
&AllocateRegister( \$projPos, "\$projPos" );																								
push @output, "; Transform position from object to projection space																								; LINEINFO(WaterCheap_vs11.vsh)(20)\n";
push @output, "dp4 $projPos.x, $vPos, $cModelViewProj0																								; LINEINFO(WaterCheap_vs11.vsh)(21)\n";
push @output, "dp4 $projPos.y, $vPos, $cModelViewProj1																								; LINEINFO(WaterCheap_vs11.vsh)(22)\n";
push @output, "dp4 $projPos.z, $vPos, $cModelViewProj2																								; LINEINFO(WaterCheap_vs11.vsh)(23)\n";
push @output, "dp4 $projPos.w, $vPos, $cModelViewProj3																								; LINEINFO(WaterCheap_vs11.vsh)(24)\n";
push @output, "mov oPos, $projPos																								; LINEINFO(WaterCheap_vs11.vsh)(26)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(WaterCheap_vs11.vsh)(28)\n";
push @output, "; Fog																								; LINEINFO(WaterCheap_vs11.vsh)(29)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(WaterCheap_vs11.vsh)(30)\n";
&CalcFog( $worldPos, $projPos );																								
&FreeRegister( \$projPos, "\$projPos" );																								
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(WaterCheap_vs11.vsh)(35)\n";
push @output, "; Lighting																								; LINEINFO(WaterCheap_vs11.vsh)(36)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(WaterCheap_vs11.vsh)(37)\n";
push @output, "; Transform tangent space basis vectors to env map space (world space)																								; LINEINFO(WaterCheap_vs11.vsh)(39)\n";
push @output, "; This will produce a set of vectors mapping from tangent space to env space																								; LINEINFO(WaterCheap_vs11.vsh)(40)\n";
push @output, "; We'll use this to transform normals from the normal map from tangent space																								; LINEINFO(WaterCheap_vs11.vsh)(41)\n";
push @output, "; to environment map space. 																								; LINEINFO(WaterCheap_vs11.vsh)(42)\n";
push @output, "; NOTE: use dp3 here since the basis vectors are vectors, not points																								; LINEINFO(WaterCheap_vs11.vsh)(43)\n";
push @output, "dp3 oT1.x, $vTangentS, $cModel0																								; LINEINFO(WaterCheap_vs11.vsh)(45)\n";
push @output, "dp3 oT2.x, $vTangentS, $cModel1																								; LINEINFO(WaterCheap_vs11.vsh)(46)\n";
push @output, "dp3 oT3.x, $vTangentS, $cModel2																								; LINEINFO(WaterCheap_vs11.vsh)(47)\n";
push @output, "dp3 oT1.y, $vTangentT, $cModel0																								; LINEINFO(WaterCheap_vs11.vsh)(49)\n";
push @output, "dp3 oT2.y, $vTangentT, $cModel1																								; LINEINFO(WaterCheap_vs11.vsh)(50)\n";
push @output, "dp3 oT3.y, $vTangentT, $cModel2																								; LINEINFO(WaterCheap_vs11.vsh)(51)\n";
push @output, "dp3 oT1.z, $vNormal, $cModel0																								; LINEINFO(WaterCheap_vs11.vsh)(53)\n";
push @output, "dp3 oT2.z, $vNormal, $cModel1																								; LINEINFO(WaterCheap_vs11.vsh)(54)\n";
push @output, "dp3 oT3.z, $vNormal, $cModel2																								; LINEINFO(WaterCheap_vs11.vsh)(55)\n";
push @output, "; Compute the vector from vertex to camera																								; LINEINFO(WaterCheap_vs11.vsh)(57)\n";
&AllocateRegister( \$worldEyeVect, "\$worldEyeVect" );																								
push @output, "sub $worldEyeVect.xyz, $cEyePos, $worldPos																								; LINEINFO(WaterCheap_vs11.vsh)(59)\n";
&FreeRegister( \$worldPos, "\$worldPos" );																								
push @output, "; Move it into the w component of the texture coords, as the wacky																								; LINEINFO(WaterCheap_vs11.vsh)(62)\n";
push @output, "; pixel shader wants it there.																								; LINEINFO(WaterCheap_vs11.vsh)(63)\n";
push @output, "mov oT1.w, $worldEyeVect.x																								; LINEINFO(WaterCheap_vs11.vsh)(64)\n";
push @output, "mov oT2.w, $worldEyeVect.y																								; LINEINFO(WaterCheap_vs11.vsh)(65)\n";
push @output, "mov oT3.w, $worldEyeVect.z																								; LINEINFO(WaterCheap_vs11.vsh)(66)\n";
push @output, "; Get the magnitude of worldEyeVect																								; LINEINFO(WaterCheap_vs11.vsh)(68)\n";
push @output, "dp3 $worldEyeVect.w, $worldEyeVect, $worldEyeVect																								; LINEINFO(WaterCheap_vs11.vsh)(69)\n";
push @output, "rsq $worldEyeVect.w, $worldEyeVect.w																								; LINEINFO(WaterCheap_vs11.vsh)(70)\n";
push @output, "rcp $worldEyeVect.w, $worldEyeVect.w																								; LINEINFO(WaterCheap_vs11.vsh)(71)\n";
push @output, "; calculate the cheap water blend factor and stick it into oD0.a																								; LINEINFO(WaterCheap_vs11.vsh)(73)\n";
push @output, "; NOTE: This won't be perspective correct!!!!!																								; LINEINFO(WaterCheap_vs11.vsh)(74)\n";
push @output, "; OPTIMIZE: This could turn into a mad.																								; LINEINFO(WaterCheap_vs11.vsh)(75)\n";
push @output, "add $worldEyeVect.w, $worldEyeVect.w, -$SHADER_SPECIFIC_CONST_2.x																								; LINEINFO(WaterCheap_vs11.vsh)(76)\n";
push @output, "mul oD0, $worldEyeVect.w, $SHADER_SPECIFIC_CONST_2.y																								; LINEINFO(WaterCheap_vs11.vsh)(77)\n";
&FreeRegister( \$worldEyeVect, "\$worldEyeVect" );																								
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(WaterCheap_vs11.vsh)(81)\n";
push @output, "; Texture coordinates																								; LINEINFO(WaterCheap_vs11.vsh)(82)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(WaterCheap_vs11.vsh)(83)\n";
push @output, "dp4 oT0.x, $vTexCoord0, $SHADER_SPECIFIC_CONST_0																								; LINEINFO(WaterCheap_vs11.vsh)(84)\n";
push @output, "dp4 oT0.y, $vTexCoord0, $SHADER_SPECIFIC_CONST_1																								; LINEINFO(WaterCheap_vs11.vsh)(85)\n";
