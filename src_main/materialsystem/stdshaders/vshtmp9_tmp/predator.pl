...
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(predator.vsh)(6)\n";
push @output, "; Constants specified by the app																								; LINEINFO(predator.vsh)(7)\n";
     push @output, ";    c0      = (0, 1, 2, 0.5)																								; LINEINFO(predator.vsh)(8)\n";
 	 push @output, ";	 c1		 = (1/2.2, 0, 0, 0)																								; LINEINFO(predator.vsh)(9)\n";
     push @output, ";    c2      = camera position *in world space*																								; LINEINFO(predator.vsh)(10)\n";
     push @output, ";    c4-c7   = modelViewProj matrix	(transpose)																								; LINEINFO(predator.vsh)(11)\n";
     push @output, ";    c8-c11  = ViewProj matrix (transpose)																								; LINEINFO(predator.vsh)(12)\n";
     push @output, ";    c12-c15 = model->view matrix (transpose)																								; LINEINFO(predator.vsh)(13)\n";
 	 push @output, ";	 c16	 = [fogStart, fogEnd, fogRange, undefined]																								; LINEINFO(predator.vsh)(14)\n";
 																								push @output, ";																								; LINEINFO(predator.vsh)(15)\n";
push @output, "; Vertex components (as specified in the vertex DECL)																								; LINEINFO(predator.vsh)(16)\n";
     push @output, ";    $vPos    = Position																								; LINEINFO(predator.vsh)(17)\n";
 	 push @output, ";	 $vTexCoord0.xy = TexCoord0																								; LINEINFO(predator.vsh)(18)\n";
...
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(predator.vsh)(23)\n";
push @output, "; Vertex blending 																								; LINEINFO(predator.vsh)(24)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(predator.vsh)(25)\n";
&AllocateRegister( \$worldPos, "\$worldPos" );																								
&AllocateRegister( \$worldNormal, "\$worldNormal" );																								
&SkinPositionAndNormal( $worldPos, $worldNormal );																								
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(predator.vsh)(30)\n";
push @output, "; Transform the position from world to proj space																								; LINEINFO(predator.vsh)(31)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(predator.vsh)(32)\n";
&AllocateRegister( \$projPos, "\$projPos" );																								
push @output, "dp4 $projPos.x, $worldPos, $cViewProj0																								; LINEINFO(predator.vsh)(36)\n";
push @output, "dp4 $projPos.y, $worldPos, $cViewProj1																								; LINEINFO(predator.vsh)(37)\n";
push @output, "dp4 $projPos.z, $worldPos, $cViewProj2																								; LINEINFO(predator.vsh)(38)\n";
push @output, "dp4 $projPos.w, $worldPos, $cViewProj3																								; LINEINFO(predator.vsh)(39)\n";
push @output, "mov oPos, $projPos																								; LINEINFO(predator.vsh)(40)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(predator.vsh)(43)\n";
push @output, "; Fog																								; LINEINFO(predator.vsh)(44)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(predator.vsh)(45)\n";
&CalcFog( $worldPos, $projPos );																								
&FreeRegister( \$worldPos, "\$worldPos" );																								
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(predator.vsh)(51)\n";
push @output, "; Transform the normal from world to proj space																								; LINEINFO(predator.vsh)(52)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(predator.vsh)(53)\n";
&AllocateRegister( \$projNormal, "\$projNormal" );																								
push @output, "; only do X and Y since that's all we care about																								; LINEINFO(predator.vsh)(57)\n";
push @output, "dp3 $projNormal.x, $worldNormal, $cViewProj0																								; LINEINFO(predator.vsh)(58)\n";
push @output, "dp3 $projNormal.y, $worldNormal, $cViewProj1																								; LINEINFO(predator.vsh)(59)\n";
&FreeRegister( \$worldNormal, "\$worldNormal" );																								
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(predator.vsh)(63)\n";
push @output, "; Texture coordinates																								; LINEINFO(predator.vsh)(64)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(predator.vsh)(65)\n";
push @output, "; NOTE: projPos isn't projPos after this point. :)																								; LINEINFO(predator.vsh)(67)\n";
push @output, "; divide by z																								; LINEINFO(predator.vsh)(69)\n";
push @output, "rcp $projPos.w, $projPos.w																								; LINEINFO(predator.vsh)(70)\n";
push @output, "mul $projPos.xy, $projPos.w, $projPos.xy																								; LINEINFO(predator.vsh)(71)\n";
push @output, "; map from -1..1 to 0..1																								; LINEINFO(predator.vsh)(73)\n";
push @output, "mad $projPos.xy, $projPos.xy, $cHalf, $cHalf																								; LINEINFO(predator.vsh)(74)\n";
push @output, "; tweak with the texcoords based on the normal and $refractionamount																								; LINEINFO(predator.vsh)(76)\n";
push @output, "mad $projPos.xy, $projNormal.xy, -$SHADER_SPECIFIC_CONST_4.xy, $projPos.xy		; FIXME																								; LINEINFO(predator.vsh)(77)\n";
push @output, "; invert y																								; LINEINFO(predator.vsh)(79)\n";
push @output, "add $projPos.y, $cOne, -$projPos.y																								; LINEINFO(predator.vsh)(80)\n";
push @output, "; hack scale for nvidia (Power of two texcoords are screwed.)																								; LINEINFO(predator.vsh)(82)\n";
push @output, "mul oT0.xy, $projPos.xy, $SHADER_SPECIFIC_CONST_5.xy																								; LINEINFO(predator.vsh)(83)\n";
&FreeRegister( \$projPos, "\$projPos" );																								
