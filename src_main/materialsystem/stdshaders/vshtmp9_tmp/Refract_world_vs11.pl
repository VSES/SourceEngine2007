...
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Refract_world_vs11.vsh)(5)\n";
push @output, "; Constants specified by the app																								; LINEINFO(Refract_world_vs11.vsh)(6)\n";
     push @output, ";    c0      = (0, 1, 2, 0.5)																								; LINEINFO(Refract_world_vs11.vsh)(7)\n";
 	 push @output, ";	 c1		 = (1/2.2, 0, 0, 0)																								; LINEINFO(Refract_world_vs11.vsh)(8)\n";
     push @output, ";    c2      = camera position *in world space*																								; LINEINFO(Refract_world_vs11.vsh)(9)\n";
     push @output, ";    c4-c7   = modelViewProj matrix	(transpose)																								; LINEINFO(Refract_world_vs11.vsh)(10)\n";
     push @output, ";    c8-c11  = ViewProj matrix (transpose)																								; LINEINFO(Refract_world_vs11.vsh)(11)\n";
     push @output, ";    c12-c15 = model->view matrix (transpose)																								; LINEINFO(Refract_world_vs11.vsh)(12)\n";
 	 push @output, ";	 c16	 = [fogStart, fogEnd, fogRange, undefined]																								; LINEINFO(Refract_world_vs11.vsh)(13)\n";
 																								push @output, ";																								; LINEINFO(Refract_world_vs11.vsh)(14)\n";
push @output, "; Vertex components (as specified in the vertex DECL)																								; LINEINFO(Refract_world_vs11.vsh)(15)\n";
     push @output, ";    $vPos    = Position																								; LINEINFO(Refract_world_vs11.vsh)(16)\n";
 	 push @output, ";	 $vTexCoord0.xy = TexCoord0																								; LINEINFO(Refract_world_vs11.vsh)(17)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Refract_world_vs11.vsh)(18)\n";
...
push @output, "; Vertex components																								; LINEINFO(Refract_world_vs11.vsh)(22)\n";
     push @output, ";    $vPos		= Position																								; LINEINFO(Refract_world_vs11.vsh)(23)\n";
 	 push @output, ";	 $vNormal		= normal																								; LINEINFO(Refract_world_vs11.vsh)(24)\n";
 	 push @output, ";	 $vTexCoord0.xy	= TexCoord0																								; LINEINFO(Refract_world_vs11.vsh)(25)\n";
 	 push @output, ";	 $vTangentS		= S axis of Texture space																								; LINEINFO(Refract_world_vs11.vsh)(26)\n";
 	 push @output, ";	 $vTangentT	= T axis of Texture space																								; LINEINFO(Refract_world_vs11.vsh)(27)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Refract_world_vs11.vsh)(29)\n";
push @output, "; Transform the position from world to view space																								; LINEINFO(Refract_world_vs11.vsh)(30)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Refract_world_vs11.vsh)(31)\n";
...
push @output, "dp4 $projPos.x, $vPos, $cModelViewProj0																								; LINEINFO(Refract_world_vs11.vsh)(39)\n";
push @output, "dp4 $projPos.y, $vPos, $cModelViewProj1																								; LINEINFO(Refract_world_vs11.vsh)(40)\n";
push @output, "dp4 $projPos.z, $vPos, $cModelViewProj2																								; LINEINFO(Refract_world_vs11.vsh)(41)\n";
push @output, "dp4 $projPos.w, $vPos, $cModelViewProj3																								; LINEINFO(Refract_world_vs11.vsh)(42)\n";
push @output, "mov oPos, $projPos																								; LINEINFO(Refract_world_vs11.vsh)(43)\n";
push @output, "dp3 $worldPos.x, $vPos, $cModel0																								; LINEINFO(Refract_world_vs11.vsh)(45)\n";
push @output, "dp3 $worldPos.y, $vPos, $cModel1																								; LINEINFO(Refract_world_vs11.vsh)(46)\n";
push @output, "dp3 $worldPos.z, $vPos, $cModel2																								; LINEINFO(Refract_world_vs11.vsh)(47)\n";
push @output, "dp3 $worldNormal.x, $vNormal, $cModel0																								; LINEINFO(Refract_world_vs11.vsh)(49)\n";
push @output, "dp3 $worldNormal.y, $vNormal, $cModel1																								; LINEINFO(Refract_world_vs11.vsh)(50)\n";
push @output, "dp3 $worldNormal.z, $vNormal, $cModel2																								; LINEINFO(Refract_world_vs11.vsh)(51)\n";
push @output, "dp3 $worldTangentS.x, $vTangentS, $cModel0																								; LINEINFO(Refract_world_vs11.vsh)(53)\n";
push @output, "dp3 $worldTangentS.y, $vTangentS, $cModel1																								; LINEINFO(Refract_world_vs11.vsh)(54)\n";
push @output, "dp3 $worldTangentS.z, $vTangentS, $cModel2																								; LINEINFO(Refract_world_vs11.vsh)(55)\n";
push @output, "dp3 $worldTangentT.x, $vTangentT, $cModel0																								; LINEINFO(Refract_world_vs11.vsh)(57)\n";
push @output, "dp3 $worldTangentT.y, $vTangentT, $cModel1																								; LINEINFO(Refract_world_vs11.vsh)(58)\n";
push @output, "dp3 $worldTangentT.z, $vTangentT, $cModel2																								; LINEINFO(Refract_world_vs11.vsh)(59)\n";
&CalcFog( $worldPos, $projPos );																								
local( $worldEyeVect ); &AllocateRegister( \$worldEyeVect, "\$worldEyeVect" );
push @output, "; Get the eye vector in world space																								; LINEINFO(Refract_world_vs11.vsh)(65)\n";
push @output, "add $worldEyeVect.xyz, -$worldPos, $cEyePos																								; LINEINFO(Refract_world_vs11.vsh)(66)\n";
...
push @output, "; transform the eye vector to tangent space																								; LINEINFO(Refract_world_vs11.vsh)(71)\n";
push @output, "dp3 $tangentEyeVect.x, $worldEyeVect, $worldTangentS																								; LINEINFO(Refract_world_vs11.vsh)(72)\n";
push @output, "dp3 $tangentEyeVect.y, $worldEyeVect, $worldTangentT																								; LINEINFO(Refract_world_vs11.vsh)(73)\n";
push @output, "dp3 $tangentEyeVect.z, $worldEyeVect, $worldNormal																								; LINEINFO(Refract_world_vs11.vsh)(74)\n";
&Normalize( $tangentEyeVect );																								
push @output, "; stick the tangent space eye vector into oD0																								; LINEINFO(Refract_world_vs11.vsh)(78)\n";
push @output, "mad oD0.xyz, $tangentEyeVect, $cHalf, $cHalf																								; LINEINFO(Refract_world_vs11.vsh)(79)\n";
push @output, "dp4 $bumpTexCoord.x, $vTexCoord0, $SHADER_SPECIFIC_CONST_1																								; LINEINFO(Refract_world_vs11.vsh)(81)\n";
push @output, "dp4 $bumpTexCoord.y, $vTexCoord0, $SHADER_SPECIFIC_CONST_2																								; LINEINFO(Refract_world_vs11.vsh)(82)\n";
push @output, "; dudv map																								; LINEINFO(Refract_world_vs11.vsh)(84)\n";
push @output, "mov oT0.xy, $bumpTexCoord																								; LINEINFO(Refract_world_vs11.vsh)(85)\n";
push @output, "; refract tint																								; LINEINFO(Refract_world_vs11.vsh)(87)\n";
push @output, "mov oT3.xy, $bumpTexCoord																								; LINEINFO(Refract_world_vs11.vsh)(88)\n";
...
push @output, "mov oPos, $projPos																								; LINEINFO(Refract_world_vs11.vsh)(95)\n";
push @output, "; special case perspective correct texture projection so that the texture fits exactly on the screen																								; LINEINFO(Refract_world_vs11.vsh)(97)\n";
push @output, "mul $projPos.y, $projPos.y, $SHADER_SPECIFIC_CONST_4.w																								; LINEINFO(Refract_world_vs11.vsh)(98)\n";
push @output, "add $projPos.xy, $projPos.xy, $projPos.w																								; LINEINFO(Refract_world_vs11.vsh)(99)\n";
push @output, "mul $projPos.xy, $projPos.xy, $cHalf																								; LINEINFO(Refract_world_vs11.vsh)(100)\n";
push @output, "; Do the perspective divide here. .yuck . . we aren't going to be perspective correct																								; LINEINFO(Refract_world_vs11.vsh)(102)\n";
push @output, "rcp $w.w, $projPos.w																								; LINEINFO(Refract_world_vs11.vsh)(103)\n";
push @output, "mul $projPos, $projPos, $w.w																								; LINEINFO(Refract_world_vs11.vsh)(104)\n";
#max $projPos.x, $projPos.x, -$cOne																								
#min $projPos.x, $projPos.x, $cOne																								
#max $projPos.z, $projPos.z, $cZero																								
#min $projPos.z, $projPos.z, $cOne																								
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Refract_world_vs11.vsh)(111)\n";
push @output, "; Transform the tangentS from world to view space																								; LINEINFO(Refract_world_vs11.vsh)(112)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Refract_world_vs11.vsh)(113)\n";
local( $projTangentS ); &AllocateRegister( \$projTangentS, "\$projTangentS" );
push @output, "; we only care about x and y																								; LINEINFO(Refract_world_vs11.vsh)(117)\n";
push @output, "dp3 $projTangentS.x, $worldTangentS, $cViewProj0																								; LINEINFO(Refract_world_vs11.vsh)(118)\n";
push @output, "dp3 $projTangentS.y, $worldTangentS, $cViewProj1																								; LINEINFO(Refract_world_vs11.vsh)(119)\n";
push @output, "; project tangentS																								; LINEINFO(Refract_world_vs11.vsh)(121)\n";
push @output, "mul $projTangentS.xy, $projTangentS.xy, $w.w																								; LINEINFO(Refract_world_vs11.vsh)(122)\n";
push @output, ";max $projTangentS.xy, $projTangentS.xy, $cOne																								; LINEINFO(Refract_world_vs11.vsh)(124)\n";
push @output, ";min $projTangentS.xy, $projTangentS.xy, -$cOne																								; LINEINFO(Refract_world_vs11.vsh)(125)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Refract_world_vs11.vsh)(127)\n";
push @output, "; Transform the tangentT from world to view space																								; LINEINFO(Refract_world_vs11.vsh)(128)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Refract_world_vs11.vsh)(129)\n";
local( $projTangentT ); &AllocateRegister( \$projTangentT, "\$projTangentT" );
local( $texCoord ); &AllocateRegister( \$texCoord, "\$texCoord" );
push @output, "; we only care about x and y																								; LINEINFO(Refract_world_vs11.vsh)(134)\n";
push @output, "dp3 $projTangentT.x, $worldTangentT, $cViewProj0																								; LINEINFO(Refract_world_vs11.vsh)(135)\n";
push @output, "dp3 $projTangentT.y, $worldTangentT, $cViewProj1																								; LINEINFO(Refract_world_vs11.vsh)(136)\n";
push @output, "; project tangentT																								; LINEINFO(Refract_world_vs11.vsh)(138)\n";
push @output, "mul $projTangentT.xy, $projTangentT.xy, $w.w																								; LINEINFO(Refract_world_vs11.vsh)(139)\n";
push @output, ";max $projTangentT.xy, $projTangentT.xy, $cOne																								; LINEINFO(Refract_world_vs11.vsh)(141)\n";
push @output, ";min $projTangentT.xy, $projTangentT.xy, -$cOne																								; LINEINFO(Refract_world_vs11.vsh)(142)\n";
push @output, ";max $projPos.xy, $projPos.xy, $cOne																								; LINEINFO(Refract_world_vs11.vsh)(144)\n";
push @output, ";min $projPos.xy, $projPos.xy, -$cOne																								; LINEINFO(Refract_world_vs11.vsh)(145)\n";
push @output, "mul oT1.x, $projTangentS.x, $SHADER_SPECIFIC_CONST_3.x																								; LINEINFO(Refract_world_vs11.vsh)(147)\n";
push @output, "mul oT1.y, $projTangentT.x, $SHADER_SPECIFIC_CONST_3.x																								; LINEINFO(Refract_world_vs11.vsh)(148)\n";
push @output, "mov oT1.z, $projPos.x ; huh?																								; LINEINFO(Refract_world_vs11.vsh)(149)\n";
push @output, "mul $texCoord.x, $projTangentS.y, -$SHADER_SPECIFIC_CONST_3.x																								; LINEINFO(Refract_world_vs11.vsh)(151)\n";
push @output, "mul $texCoord.y, $projTangentT.y, -$SHADER_SPECIFIC_CONST_3.x																								; LINEINFO(Refract_world_vs11.vsh)(152)\n";
push @output, "mov $texCoord.z, $projPos.y																								; LINEINFO(Refract_world_vs11.vsh)(153)\n";
push @output, "mov oT2.xyz, $texCoord																								; LINEINFO(Refract_world_vs11.vsh)(154)\n";
push @output, "mov oT3.xyz, $texCoord																								; LINEINFO(Refract_world_vs11.vsh)(155)\n";
