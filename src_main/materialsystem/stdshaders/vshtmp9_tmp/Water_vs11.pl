...
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Water_vs11.vsh)(5)\n";
push @output, "; Constants specified by the app																								; LINEINFO(Water_vs11.vsh)(6)\n";
     push @output, ";    c0      = (0, 1, 2, 0.5)																								; LINEINFO(Water_vs11.vsh)(7)\n";
 	 push @output, ";	 c1		 = (1/2.2, 0, 0, 0)																								; LINEINFO(Water_vs11.vsh)(8)\n";
     push @output, ";    c2      = camera position *in world space*																								; LINEINFO(Water_vs11.vsh)(9)\n";
     push @output, ";    c4-c7   = modelViewProj matrix	(transpose)																								; LINEINFO(Water_vs11.vsh)(10)\n";
     push @output, ";    c8-c11  = ViewProj matrix (transpose)																								; LINEINFO(Water_vs11.vsh)(11)\n";
     push @output, ";    c12-c15 = model->view matrix (transpose)																								; LINEINFO(Water_vs11.vsh)(12)\n";
 	 push @output, ";	 c16	 = [fogStart, fogEnd, fogRange, undefined]																								; LINEINFO(Water_vs11.vsh)(13)\n";
 																								push @output, ";																								; LINEINFO(Water_vs11.vsh)(14)\n";
push @output, "; Vertex components (as specified in the vertex DECL)																								; LINEINFO(Water_vs11.vsh)(15)\n";
     push @output, ";    $vPos    = Position																								; LINEINFO(Water_vs11.vsh)(16)\n";
 	 push @output, ";	 $vTexCoord0.xy = TexCoord0																								; LINEINFO(Water_vs11.vsh)(17)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Water_vs11.vsh)(18)\n";
...
push @output, "; Vertex components																								; LINEINFO(Water_vs11.vsh)(22)\n";
     push @output, ";    $vPos		= Position																								; LINEINFO(Water_vs11.vsh)(23)\n";
 	 push @output, ";	 $vNormal		= normal																								; LINEINFO(Water_vs11.vsh)(24)\n";
 	 push @output, ";	 $vTexCoord0.xy	= TexCoord0																								; LINEINFO(Water_vs11.vsh)(25)\n";
 	 push @output, ";	 $vTangentS		= S axis of Texture space																								; LINEINFO(Water_vs11.vsh)(26)\n";
 	 push @output, ";	 $vTangentT	= T axis of Texture space																								; LINEINFO(Water_vs11.vsh)(27)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Water_vs11.vsh)(29)\n";
push @output, "; Transform the position from world to view space																								; LINEINFO(Water_vs11.vsh)(30)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Water_vs11.vsh)(31)\n";
local( $projPos ); &AllocateRegister( \$projPos, "\$projPos" );
push @output, "; Transform position from object to projection space																								; LINEINFO(Water_vs11.vsh)(35)\n";
push @output, "dp4 $projPos.x, $vPos, $cModelViewProj0																								; LINEINFO(Water_vs11.vsh)(36)\n";
push @output, "dp4 $projPos.y, $vPos, $cModelViewProj1																								; LINEINFO(Water_vs11.vsh)(37)\n";
push @output, "dp4 $projPos.z, $vPos, $cModelViewProj2																								; LINEINFO(Water_vs11.vsh)(38)\n";
push @output, "dp4 $projPos.w, $vPos, $cModelViewProj3																								; LINEINFO(Water_vs11.vsh)(39)\n";
local( $worldPos ); &AllocateRegister( \$worldPos, "\$worldPos" );
push @output, "; Transform position from object to world space																								; LINEINFO(Water_vs11.vsh)(43)\n";
push @output, "dp4 $worldPos.x, $vPos, $cModel0																								; LINEINFO(Water_vs11.vsh)(44)\n";
push @output, "dp4 $worldPos.y, $vPos, $cModel1																								; LINEINFO(Water_vs11.vsh)(45)\n";
push @output, "dp4 $worldPos.z, $vPos, $cModel2																								; LINEINFO(Water_vs11.vsh)(46)\n";
&CalcFog( $worldPos, $projPos );																								
local( $worldEyeVect ); &AllocateRegister( \$worldEyeVect, "\$worldEyeVect" );
push @output, "; Get the eye vector in world space																								; LINEINFO(Water_vs11.vsh)(52)\n";
push @output, "add $worldEyeVect.xyz, -$worldPos, $cEyePos																								; LINEINFO(Water_vs11.vsh)(53)\n";
push @output, "; transform the eye vector to tangent space																								; LINEINFO(Water_vs11.vsh)(55)\n";
push @output, "dp3 oT3.x, $worldEyeVect, $vTangentS																								; LINEINFO(Water_vs11.vsh)(56)\n";
push @output, "dp3 oT3.y, $worldEyeVect, $vTangentT																								; LINEINFO(Water_vs11.vsh)(57)\n";
push @output, "dp3 oT3.z, $worldEyeVect, $vNormal																								; LINEINFO(Water_vs11.vsh)(58)\n";
push @output, "; Get the magnitude of worldEyeVect																								; LINEINFO(Water_vs11.vsh)(60)\n";
push @output, "dp3 $worldEyeVect.w, $worldEyeVect, $worldEyeVect																								; LINEINFO(Water_vs11.vsh)(61)\n";
push @output, "rsq $worldEyeVect.w, $worldEyeVect.w																								; LINEINFO(Water_vs11.vsh)(62)\n";
push @output, "rcp $worldEyeVect.w, $worldEyeVect.w																								; LINEINFO(Water_vs11.vsh)(63)\n";
push @output, "; calculate the cheap water blend factor and stick it into oD0.a																								; LINEINFO(Water_vs11.vsh)(65)\n";
push @output, "; NOTE: This won't be perspective correct!!!!!																								; LINEINFO(Water_vs11.vsh)(66)\n";
push @output, "; OPTIMIZE: This could turn into a mad.																								; LINEINFO(Water_vs11.vsh)(67)\n";
push @output, "add $worldEyeVect.w, $worldEyeVect.w, -$SHADER_SPECIFIC_CONST_3.x																								; LINEINFO(Water_vs11.vsh)(68)\n";
push @output, "mul oD0, $worldEyeVect.w, $SHADER_SPECIFIC_CONST_3.y																								; LINEINFO(Water_vs11.vsh)(69)\n";
push @output, "; dudv map																								; LINEINFO(Water_vs11.vsh)(71)\n";
local( $bumpTexCoord ); &AllocateRegister( \$bumpTexCoord, "\$bumpTexCoord" );
push @output, "dp4 $bumpTexCoord.x, $vTexCoord0, $SHADER_SPECIFIC_CONST_1																								; LINEINFO(Water_vs11.vsh)(74)\n";
push @output, "dp4 $bumpTexCoord.y, $vTexCoord0, $SHADER_SPECIFIC_CONST_2																								; LINEINFO(Water_vs11.vsh)(75)\n";
push @output, "mov oT0.xy, $bumpTexCoord																								; LINEINFO(Water_vs11.vsh)(77)\n";
push @output, "; normal map																								; LINEINFO(Water_vs11.vsh)(79)\n";
push @output, "mov oT2.xy, $bumpTexCoord																								; LINEINFO(Water_vs11.vsh)(80)\n";
&FreeRegister( \$bumpTexCoord, "\$bumpTexCoord" );
local( $newProjPos ); &AllocateRegister( \$newProjPos, "\$newProjPos" );
push @output, "mov oPos, $projPos																								; LINEINFO(Water_vs11.vsh)(86)\n";
push @output, "; special case perspective correct texture projection so that the texture fits exactly on the screen																								; LINEINFO(Water_vs11.vsh)(88)\n";
push @output, "mul $projPos.y, $projPos.y, $SHADER_SPECIFIC_CONST_4.w																								; LINEINFO(Water_vs11.vsh)(89)\n";
push @output, "add $projPos.xy, $projPos.xy, $projPos.w																								; LINEINFO(Water_vs11.vsh)(90)\n";
push @output, "mul $projPos.xy, $projPos.xy, $cHalf																								; LINEINFO(Water_vs11.vsh)(91)\n";
push @output, "mov oT1.xy, $projPos.xy																								; LINEINFO(Water_vs11.vsh)(93)\n";
push @output, "mov oT1.z, $cZero																								; LINEINFO(Water_vs11.vsh)(94)\n";
push @output, "mov oT1.w, $projPos.w																								; LINEINFO(Water_vs11.vsh)(95)\n";
&FreeRegister( \$projPos, "\$projPos" );
