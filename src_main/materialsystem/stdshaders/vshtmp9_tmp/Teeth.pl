...
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(9)\n";
 	 push @output, ";	 $SHADER_SPECIFIC_CONST_0	 = xyz = mouth forward direction vector, w = illum factor																								; LINEINFO(Teeth.vsh)(10)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(11)\n";
...
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(18)\n";
push @output, "; Vertex blending 																								; LINEINFO(Teeth.vsh)(19)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(20)\n";
...
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(25)\n";
push @output, "; Optional intro warping																								; LINEINFO(Teeth.vsh)(26)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(27)\n";
if ( $INTRO == 1 )																								
{																								
	local( $tmp ); &AllocateRegister( \$tmp, "\$tmp" );
	push @output, "	sub $tmp.xyz, $worldPos, $ENTITY_ORIGIN																								; LINEINFO(Teeth.vsh)(31)\n";
	push @output, "	mul $tmp.xy, $tmp, $WARPPARAM																								; LINEINFO(Teeth.vsh)(32)\n";
	push @output, "	add $worldPos.xyz, $tmp, $ENTITY_ORIGIN																								; LINEINFO(Teeth.vsh)(33)\n";
	&FreeRegister( \$tmp, "\$tmp" );
}																								
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(37)\n";
push @output, "; Transform the position from world to view space																								; LINEINFO(Teeth.vsh)(38)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(39)\n";
local( $projPos ); &AllocateRegister( \$projPos, "\$projPos" );
push @output, "dp4 $projPos.x, $worldPos, $cViewProj0																								; LINEINFO(Teeth.vsh)(43)\n";
push @output, "dp4 $projPos.y, $worldPos, $cViewProj1																								; LINEINFO(Teeth.vsh)(44)\n";
push @output, "dp4 $projPos.z, $worldPos, $cViewProj2																								; LINEINFO(Teeth.vsh)(45)\n";
push @output, "dp4 $projPos.w, $worldPos, $cViewProj3																								; LINEINFO(Teeth.vsh)(46)\n";
push @output, "mov oPos, $projPos																								; LINEINFO(Teeth.vsh)(47)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(49)\n";
push @output, "; Fog																								; LINEINFO(Teeth.vsh)(50)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(51)\n";
&CalcFog( $worldPos, $projPos );																								
&FreeRegister( \$projPos, "\$projPos" );
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(57)\n";
push @output, "; Lighting																								; LINEINFO(Teeth.vsh)(58)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(59)\n";
local( $linearColor ); &AllocateRegister( \$linearColor, "\$linearColor" );
&DoDynamicLightingToLinear( $worldPos, $worldNormal, $linearColor );																								
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(63)\n";
push @output, "; Factor in teeth darkening factors																								; LINEINFO(Teeth.vsh)(64)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(65)\n";
local( $tmp ); &AllocateRegister( \$tmp, "\$tmp" );
push @output, "mul $linearColor.xyz, $SHADER_SPECIFIC_CONST_0.w, $linearColor	; FIXME Color darkened by illumination factor																								; LINEINFO(Teeth.vsh)(69)\n";
push @output, "dp3 $tmp, $worldNormal, $SHADER_SPECIFIC_CONST_0					; Figure out mouth forward dot normal																								; LINEINFO(Teeth.vsh)(70)\n";
push @output, "max	$tmp, $cZero, $tmp						; clamp from 0 to 1																								; LINEINFO(Teeth.vsh)(71)\n";
push @output, "mul $linearColor.xyz, $tmp, $linearColor	; Darken by forward dot normal too																								; LINEINFO(Teeth.vsh)(72)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(74)\n";
push @output, "; Output color (gamma correction)																								; LINEINFO(Teeth.vsh)(75)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(76)\n";
local( $gammaColor ); &AllocateRegister( \$gammaColor, "\$gammaColor" );
&LinearToGamma( $linearColor, $gammaColor );																								
&FreeRegister( \$linearColor, "\$linearColor" );
push @output, "mul oD0.xyz, $gammaColor.xyz, $cOverbrightFactor																								; LINEINFO(Teeth.vsh)(81)\n";
push @output, "mov oD0.w, $cOne				; make sure all components are defined																								; LINEINFO(Teeth.vsh)(82)\n";
&FreeRegister( \$gammaColor, "\$gammaColor" );
&FreeRegister( \$worldPos, "\$worldPos" );
&FreeRegister( \$worldNormal, "\$worldNormal" );
&FreeRegister( \$tmp, "\$tmp" );
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(90)\n";
push @output, "; Texture coordinates																								; LINEINFO(Teeth.vsh)(91)\n";
push @output, ";------------------------------------------------------------------------------																								; LINEINFO(Teeth.vsh)(92)\n";
push @output, "mov oT0, $vTexCoord0																								; LINEINFO(Teeth.vsh)(94)\n";
