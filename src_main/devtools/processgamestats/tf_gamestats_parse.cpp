...
void InsertTF2Data( bool bDeathOnly, char const *szStatsFileUserID, time_t fileTime, IMySQL *sql, int iStatsFileVersion )
{
		...
		iMap = g_reportedStats.m_dictMapStats.Next( iMap ) )
	{
		...
		Q_snprintf( q, sizeof( q ), "INSERT into tf_mapdata (TimeSubmitted,MapName,RoundsPlayed,TotalTime,BlueWins,RedWins,Stalemates,BlueSuddenDeathWins,RedSuddenDeathWins) "
			"values (%s,\"%s\",%d,%d,%d,%d,%d,%d,%d);",
			szDate, mapname.c_str(), pCurrentMap->m_Header.m_iRoundsPlayed, pCurrentMap->m_Header.m_iTotalTime, pCurrentMap->m_Header.m_iBlueWins,
...
//		Msg( "%s\n", q );
		...
		for ( int i = TF_CLASS_UNDEFINED + 1; i < TF_CLASS_CIVILIAN; i++ )
		{
			...
			Q_snprintf( q, sizeof( q ), "INSERT into tf_classdata (MapName, TimeSubmitted,Class,Spawns,TotalTime,Score,Kills,Deaths,Assists,Captures) "
				"values (\"%s\",%s,%d,%d,%d,%d,%d,%d,%d,%d);",
				mapname.c_str(), szDate, i, classStats.iSpawns, classStats.iTotalTime, classStats.iScore, classStats.iKills, classStats.iDeaths,
...
//			Msg( "%s\n", q );
		...
		for ( int i = TF_WEAPON_NONE+1; i < TF_WEAPON_COUNT; i++ )
		{
			...
			Q_snprintf( q, sizeof( q ), "INSERT into tf_weapondata (MapName, TimeSubmitted,WeaponID,ShotsFired,ShotsFiredCrit,ShotsHit,DamageTotal,"
				"HitsWithKnownDistance,DistanceTotal) "
				"values (\"%s\",%s,%d,%d,%d,%d,%d,%d,%I64d)",
				mapname.c_str(), szDate, i, weaponStats.iShotsFired, weaponStats.iCritShotsFired, weaponStats.iHits, weaponStats.iTotalDamage,
...
int TF_ParseCustomGameStatsData( ParseContext_t *ctx )
{
	...
	{
		ProcessPerfData( ctx->mysql, "tf_perfdata", ( ( char * ) statsBuffer.Base() ) + 9 );
