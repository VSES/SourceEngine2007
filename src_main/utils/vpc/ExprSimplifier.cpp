...
static bool IsConditional( const char token )
{
	...
	if ( token == OR_OP || token == AND_OP )
	{
		...
		if ( (token & nextchar) == token )
		{
			...
			VPC_SyntaxError( "Bad expression token: %c %c", token, nextchar );
	}
...
static bool IsIdentifier( const char token )
{
	bool success = false;
	if ( token == '$' )
	{
		...
		int i = 0;
		mIdentifier[i++] = token;
		while( isalnum( mExpression[mCurPosition] ) && i < MAX_IDENTIFIER_LEN )
		{
...
static void MakeFactor( ExprTree& tree )
{
	...
	else if ( IsIdentifier( mCurToken ) )
	{
	...
	else
	{
		...
		VPC_SyntaxError( "Bad expression token: %c", mCurToken );
	}
