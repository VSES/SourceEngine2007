...
HRESULT CLCDGfx::Initialize(int nWidth, int nHeight)
{
    ...
    if(NULL == m_hDC)
    {
        LOGITRACE(_T("CLCDGfx::Initialize(): failed to create compatible DC.\n"));
        Shutdown();
        return E_FAIL;
    }
    ...
    {
        LOGITRACE(_T("CLCDGfx::Initialize(): failed to allocate bitmap info.\n"));
        Shutdown();
    ...
    if(NULL == m_hBitmap)
    {
        LOGITRACE(_T("CLCDGfx::Initialize(): failed to create bitmap.\n"));
        Shutdown();
        return E_FAIL;
    }
    ...
    {
        LOGITRACE(_T("CLCDGfx::Initialize(): failed to allocate the lcd screen structure.\n"));
        Shutdown();
        return E_OUTOFMEMORY;
    }
...
void CLCDGfx::EndDraw(void)
{
    ...
    if(NULL != m_hPrevBitmap)
    {
        ...
        LOGIASSERT(m_hPrevBitmap == m_hBitmap);
        m_hPrevBitmap = NULL;
    }
}
