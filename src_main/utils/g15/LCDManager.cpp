...
HRESULT CLCDManager::Initialize(void)
{
    ...
    if(FAILED(hRes))
    {
        LOGITRACE(_T("CLCDManager::Initialize(): failed to initialize base class.\n"));
        Shutdown();
        return hRes;
    }
    ...
    if(FAILED(hRes))
    {
        LOGITRACE(_T("CLCDManager::Initialize(): failed to initialize graphics component.\n"));
        Shutdown();
        return hRes;
    }
...
{
    LOGIASSERT(NULL != m_Gfx.GetHDC());
    LOGIASSERT(NULL != m_Gfx.GetHBITMAP());
    if((NULL == m_Gfx.GetHDC()) || (NULL == m_Gfx.GetHBITMAP()))
    {
        LOGITRACE(_T("CLCDManager::Draw(): trying to draw without successful Initialize() first.!\n"));
        return E_FAIL;
    }
...
{
	m_dwStartTime = m_dwStartTime ? m_dwStartTime : dwTimestamp;
    LOGIASSERT(m_dwStartTime != 0);
...
BOOL CLCDManager::HasExpired(DWORD dwTimestamp)
{
	m_dwStartTime = m_dwStartTime ? m_dwStartTime : dwTimestamp;
