...
{
    CloseAndDisconnect();
    if(lInitCount > 0 && 0 == InterlockedDecrement(&lInitCount))
    {
        lgLcdDeInit();
    }
    LOGIASSERT(lInitCount >= 0);
...
void CLCDOutput::ReadButtons()
{
    if(IsOpened())
    {
        ...
        if (ERROR_SUCCESS != res)
        {
            LOGITRACE(_T("lgLcdReadSoftButtons failed: unplug?\n"));
            HandleErrorFromAPI(res);
        }
...
void CLCDOutput::HandleButtonState(DWORD dwButtonState, DWORD dwButton)
{
    ...
    {
        LOGITRACE(_T("Button 0x%x released\n"), dwButton);
        OnLCDButtonUp(dwButton);
    }
    if ( !(m_dwButtonState & dwButton) && (dwButtonState & dwButton) )
    {
        LOGITRACE(_T("Button 0x%x pressed\n"), dwButton);
...
void CLCDOutput::HandleErrorFromAPI(DWORD dwRes)
{
    ...
    case ERROR_DEVICE_NOT_CONNECTED:
        LOGITRACE(_T("lgLcdAPI returned with ERROR_DEVICE_NOT_CONNECTED, closing device\n"));
        OnClosingDevice(m_hDevice);
        break;
    default:
        LOGITRACE(_T("lgLcdAPI returned with other error (0x%08x) closing device and connection\n"));
        OnClosingDevice(m_hDevice);
