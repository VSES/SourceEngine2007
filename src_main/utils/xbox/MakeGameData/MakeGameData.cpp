...
bool CompressCallback( CUtlBuffer &inputBuffer, CUtlBuffer &outputBuffer )
{
	...
	unsigned char *pCompressedOutput = LZMA_Compress( (unsigned char *)inputBuffer.Base() + inputBuffer.TellGet(), inputBuffer.TellPut() - inputBuffer.TellGet(), &compressedSize );
...
bool MakeGameDataApp::Create()
{
	...
	// Load vphysics.dll
	if ( !Sys_LoadInterface( "..\\..\\..\\game\\bin\\vphysics.dll", VPHYSICS_COLLISION_INTERFACE_VERSION, &g_pPhysicsModule, (void**)&g_pPhysicsCollision ) )
	{
...
int MakeGameDataApp::Main()
{
	...
	bool bRecurse = CommandLine()->FindParm( "-recurse" ) || CommandLine()->FindParm( "-r" );
	...
	if ( argnum )
	{
		strcpy( g_targetPath, CommandLine()->GetParm( argnum + 1 ) );
	}
