	...
	else if ( Q_strcmp( "dod_win_panel", pEventName ) == 0 )
	{
		...
		*/
		...
		if ( m_bShowTimerDefend )
		{
			...
			wchar_t time[8];
			_snwprintf( time, sizeof( time ), L"%d:%02d", minutes, seconds );
		...
		Q_snprintf( buf, sizeof(buf), "%s:", g_PR->GetPlayerName(iPlayer) );
		m_pLeftCategoryLabels[0]->SetText( buf );
		m_pLeftCategoryLabels[0]->SetVisible( iPlayer > 0 );

		SetupAvatar( "left", 1, iPlayer );

		iPlayer = event->GetInt( "left_2" );
		Q_snprintf( buf, sizeof(buf), "%s:", g_PR->GetPlayerName(iPlayer) );
		m_pLeftCategoryLabels[1]->SetText( buf );
		m_pLeftCategoryLabels[1]->SetVisible( iPlayer > 0 );

		SetupAvatar( "left", 2, iPlayer );

		iPlayer = event->GetInt( "left_3" );
		Q_snprintf( buf, sizeof(buf), "%s:", g_PR->GetPlayerName(iPlayer) );
		m_pLeftCategoryLabels[2]->SetText( buf );
		m_pLeftCategoryLabels[2]->SetVisible( iPlayer > 0 );
		...
		Q_snprintf( buf, sizeof(buf), "%s:", g_PR->GetPlayerName(iPlayer) );
		m_pRightCategoryLabels[0]->SetText( buf );
		m_pRightCategoryLabels[0]->SetVisible( iPlayer > 0 );

		SetupAvatar( "right", 1, iPlayer );

		iPlayer = event->GetInt( "right_2" );
		Q_snprintf( buf, sizeof(buf), "%s:", g_PR->GetPlayerName(iPlayer) );
		m_pRightCategoryLabels[1]->SetText( buf );
		m_pRightCategoryLabels[1]->SetVisible( iPlayer > 0 );

		SetupAvatar( "right", 2, iPlayer );

		iPlayer = event->GetInt( "right_3" );
		Q_snprintf( buf, sizeof(buf), "%s:", g_PR->GetPlayerName(iPlayer) );
		m_pRightCategoryLabels[2]->SetText( buf );
		m_pRightCategoryLabels[2]->SetVisible( iPlayer > 0 );
