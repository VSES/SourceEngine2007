...
void CHeadIconManager::DrawHeadIcons()
{
	...
	if ( cl_identiconmode.GetInt() <= 0 )
		goto finishedRendering;

	if( !m_pAlliesIconMaterial || !m_pAxisIconMaterial )
		goto finishedRendering;
	...
	{
	case TEAM_ALLIES:
		pRenderContext->Bind( m_pAlliesIconMaterial );
		break;

	case TEAM_AXIS:
		pRenderContext->Bind( m_pAxisIconMaterial );
		break;

	default:
		// no icon for spectators or team unassigned
		goto finishedRendering;
	}
	...
	for(int i=0; i < MAX_PLAYERS; i++)
	{
		...
		if ( m_PlayerDrawn.Get( pPlayer->entindex() - 1 ) == 0 )
			continue;
