...
void CHudChatLine::InsertAndColorizeText( wchar_t *buf, int clientIndex )
{
	...
	if ( m_text[0] == DOD_COLOR_PLAYERNAME ||
		m_text[0] == DOD_COLOR_AXIS ||
		m_text[0] == DOD_COLOR_ALLIES ||
		m_text[0] == DOD_COLOR_NORMAL )
	{
		while ( txt && *txt )
		{
			TextRange range;
	...
	if ( !m_textRanges.Count() && m_iNameLength > 0 && m_text[0] == COLOR_USEOLDCOLORS )
	{
		...
		range.color = GetTextColorForClient( COLOR_NORMAL, clientIndex );
		m_textRanges.AddToTail( range );
		*/

		// make (Dead) and (Team) in the player name color as well
		TextRange range;
		range.start = 0;
...
Color CHudChat::GetTextColorForClient( TextColor colorNum, int clientIndex )
{
	...
	switch ( colorNum )
	{
	case DOD_COLOR_PLAYERNAME:
		c = GetClientColor( clientIndex );
		break;

	case DOD_COLOR_ALLIES:
		c = g_PR->GetTeamColor( TEAM_ALLIES );
		break;

	case DOD_COLOR_AXIS:
		c = g_PR->GetTeamColor( TEAM_AXIS );
		break;
