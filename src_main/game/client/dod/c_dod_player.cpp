...
void C_DODRagdoll::ImpactTrace( trace_t *pTrace, int iDamageType, char *pCustomImpactName )
{
...
void C_DODPlayer::PostDataUpdate( DataUpdateType_t updateType )
{
	...
	if ( bIsLocalPlayer )
	{
		// Set a new sensitivity that is scaled by a separate compensating factor
		if( m_Shared.IsInMGDeploy() )
		{
			gHUD.m_flMouseSensitivity = deployed_mg_sensitivity.GetFloat();
		}
		else if( m_Shared.IsBazookaDeployed() )
		{
			gHUD.m_flMouseSensitivity = deployed_bazooka_sensitivity.GetFloat();
		}
	}
...
void C_DODPlayer::ProcessMuzzleFlashEvent()
{
	...
	muzzleFlashData.m_hEntity = pWeapon->GetRefEHandle();
	muzzleFlashData.m_nAttachmentIndex = 1;
	muzzleFlashData.m_nHitBox = pWeapon->GetDODWpnData().m_iMuzzleFlashType;
	muzzleFlashData.m_flMagnitude = pWeapon->GetDODWpnData().m_flMuzzleFlashScale;

	muzzleFlashData.m_flMagnitude = 0.2;
	DispatchEffect( "DOD_3rdPersonMuzzleFlash", muzzleFlashData );
...
void C_DODPlayer::CalcDODDeathCamView(Vector& eyeOrigin, QAngle& eyeAngles, float& fov)
{
	...
	if ( killer && (killer != this) ) 
	{
		...
		if ( player && player->IsAlive() )
		{
			...
			{
				VectorAdd( vecKiller, VEC_PRONE_VIEW, vecKiller );
			}
			else if( player->GetFlags() & FL_DUCKING )
			{
				VectorAdd( vecKiller, VEC_DUCK_VIEW, vecKiller );
			}
			else
			{
				VectorAdd( vecKiller, VEC_VIEW, vecKiller );
			}
		}
...
ConVar dod_max_separation_force ( "dod_max_separation_force", "256", FCVAR_CHEAT );
...
void C_DODPlayer::AvoidPlayers( CUserCmd *pCmd )
{
	...
	float flPushStrength = RemapValClamped( vecDelta.Length(), flAvoidRadius, 0, 0, dod_max_separation_force.GetInt() ); //flPushScale;
	...
	// These don't appear to do anything in TF
