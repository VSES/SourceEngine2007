...
const CDODViewVectors *CDODGameRules::GetDODViewVectors() const
{
	...
	{
		//Respawn Timers
		if( m_iNumAlliesRespawnWaves > 0 && m_AlliesRespawnQueue[m_iAlliesRespawnHead] < gpGlobals->curtime )
		{
		...
		if( m_iNumAxisRespawnWaves > 0 && m_AxisRespawnQueue[m_iAxisRespawnHead] < gpGlobals->curtime )
		{
	...
	void CDODGameRules::RespawnPlayers( bool bForceRespawn, bool bTeam /* = false */, int iTeam/* = TEAM_UNASSIGNED */ )
	{
		...
		for ( int i = 1; i <= gpGlobals->maxClients; i++ )
		{
			...
			if ( pPlayer->IsClassMenuOpen() && !pPlayer->IsAlive() )
			{
				CreateOrJoinRespawnWave( pPlayer );
				continue;
			}
	...
	void CDODGameRules::Precache( void )
	{
	}
	...
	void CDODGameRules::DeathNotice( CBasePlayer *pVictim, const CTakeDamageInfo &info )
	{
		...
		CBaseEntity *pInflictor = info.GetInflictor();
		CBaseEntity *pKiller = info.GetAttacker();
		CDODPlayer *pScorer = dynamic_cast<CDODPlayer*>( GetDeathScorer( pKiller, pInflictor ) );
