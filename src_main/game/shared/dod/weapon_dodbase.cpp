...
void CWeaponDODBase::Smack()
{
	...
	bool CWeaponDODBase::OnFireEvent( C_BaseViewModel *pViewModel, const Vector& origin, const QAngle& angles, int event, const char *options )
	{
		if( event == 5001 )
		{
			...
			{
				CEffectData data;
				data.m_fFlags = atoi( options );
				data.m_hEntity = pViewModel->GetRefEHandle();
				data.m_nAttachmentIndex = 1;


				int iOptions = atoi(options);
				int iFlashType = ( iOptions % 10 ) % NUM_MUZZLE_FLASH_TYPES;

				//3 = mg flash
				//0-2 = normal flash

				data.m_nHitBox = iFlashType;
				data.m_flMagnitude = ( iOptions / 10 ) * 0.1;

				DispatchEffect( "DOD_MuzzleFlash", data );
			}
			return true;
		}
		else if( event == 6002 )
	...
	Vector CWeaponDODBase::GetDesiredViewModelOffset( C_DODPlayer *pOwner )
	{
		...
		float flPercent = ( viewOffset.z - VEC_PRONE_VIEW.z ) / ( VEC_VIEW.z - VEC_PRONE_VIEW.z );
