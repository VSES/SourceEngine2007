...
int							g_TotalFrames;
map<const tchar *, unsigned> g_TimeSumsMap;
...
void CVProfile::SumTimes( CVProfNode *pNode, int budgetGroupID )
{
	...
	if ( GetRoot() != pNode )
	{
		if ( g_pStartNode && pNode->GetTotalCalls() > 0 && ( budgetGroupID == -1 || pNode->GetBudgetGroupID() == budgetGroupID ) )
		{
			...
			map<const tchar *, unsigned>::iterator iter;
...
{
	strncpy(m_CPUTraceFilename,filename,128);
	// _snprintf(m_CPUTraceFilename, 256, "e:\\%.128s.pix2", filename);
...
const char *CVProfile::GetCPUTraceFilename()
{
	...
	case kAllNodesInFrame_Recording:
		_snprintf(retBuf, 256, "e:\\%.128s%.4d.pix2", m_CPUTraceFilename, m_iSuccessiveTraceIndex);
		break;

	default:
		_snprintf(retBuf, 256, "e:\\%.128s.pix2", m_CPUTraceFilename);
	}
...
void CVProfile::DumpNodes( CVProfNode *pNode, int indent, bool bAverageAndCountOnly )
{
	...
	if ( !fIsRoot )
	{
		...
		if( bAverageAndCountOnly )
		{
			Msg( _T("  %10.3f %6.2f      %6d"), 
						 ( pNode->GetTotalCalls() > 0 ) ? pNode->GetTotalTime() / (double)NumFramesSampled() : 0, 
						 ( pNode->GetTotalCalls() > 0 ) ? iterTimeLessChildren->second / (double)NumFramesSampled() : 0, 
						 pNode->GetTotalCalls()  );
		}
		else
		{
			Msg( _T("  %10.3f %6.2f      %10.3f %6.2f      %10.3f %6.2f   %6d %6.2f"), 
...
static void DumpSorted( const tchar *pszHeading, double totalTime, bool (*pfnSort)( const TimeSums_t &, const TimeSums_t & ), int maxLen = 999999 )
{
    ...
    for ( i = 0; i < sortedSums.size() && i < (unsigned)maxLen; i++ )
    {
        ...
        Msg( _T("  %52.52s%12d%12.3f%12.3f%6.2f%%%12.3f%6.2f%%%12.3f%12.3f%12.3f%12.3f\n"), 
