...
CSDKLauncherDialog::CSDKLauncherDialog(vgui::Panel *parent, const char *name) : BaseClass(parent, name)
{
	...
	m_pCurrentEngineCombo = new ComboBox( this, "CurrentEngineList", 2, false );
	...
	// Hack to protect SDK-distributed DLL's from being overwritten by game cache versions when LoadLibrary is called by tools.
//	WinExec("attrib +r bin\\orangebox\\bin\\*.dll", SW_HIDE);
//	WinExec("attrib +r bin\\ep1\\bin\\*.dll", SW_HIDE);
}
...
void CSDKLauncherDialog::PopulateCurrentEngineCombo( bool bSelectLast )
{
	...
	// Add orangebox engine
	KeyValues *kv = new KeyValues( "values" );
	kv->SetString( "EngineVer", "orangebox" );
	m_pCurrentEngineCombo->AddItem( "The Orange Box", kv );
	kv->deleteThis();
	if ( 0 == V_strcmp( g_engineDir, "orangebox" ) )
	{
		nActiveEngine = 0;
	}
	...
	m_pCurrentEngineCombo->AddItem( "Half-Life 2: Episode One", kv );
	kv->deleteThis();
	if ( 0 == V_strcmp( g_engineDir, "ep1" ) )
	{
		nActiveEngine = 1;
	}
