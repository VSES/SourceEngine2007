...
void CVsMayaMasterApp::LoadPlugin( const char *pModuleName, const char *pModulePath )
{
	...
	sprintf(l_szFullpath,"%s//%s", pModulePath, pModuleName);
	
	const HMODULE hModule( LoadLibrary(l_szFullpath ) );
	if ( !hModule )
		return;
...
bool CVsMayaMasterApp::Create()
{
	...
	if ( !AddSystem( vsMasterModule, DCC_MAIN_INTERFACE_VERSION ) )
	{
		retVal = false;
	}
