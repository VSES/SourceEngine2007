	...
	const MSyntax::MArgType i_argType6)
{
	...
	if ( static_cast< int >( strlen( i_shortName ) ) > m_shortNameLength - 1 )
	{
		m_shortNameLength = strlen( i_shortName ) + 1;
	}
...
void
ValveMaya::CMSyntaxHelp::PrintHelp(
	...
	int i_lineLength)
{
	...
	for ( int gi( 0 ); gi != m_groupOrder.Count(); ++gi )
	{
		...
		if ( strcmp( m_groupOrder[ gi ], "default" ) )
		{
